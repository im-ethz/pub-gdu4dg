...
def toy_example_classification(sample_size=1000, source_domians=[1, 2, 3, 4, 5], target_domain=4, save_results=True):
    ...
    # target feature data
    mu_target, cov_target = source_parameter_list[target_index]
    ...
    x_tr = np.concatenate([x_data[i] for i in range(len(x_data)) if (i-1) in source_domians], axis=0).astype(np.float32)

    y_tr = np.concatenate([y_data[i] for i in range(len(y_data)) if (i-1) in source_domians], axis=0).astype(np.float32)
...
def random_sample_dist(sample_size=1000, dim=3, mean_magnitude=1, cov_magnitude=1):
