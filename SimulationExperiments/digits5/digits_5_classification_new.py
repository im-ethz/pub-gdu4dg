...
'''
        nohup /local/home/sfoell/anaconda3/envs/gdu_old/bin/python3.8 -u /local/home/sfoell/GitHub/gdu-old/SimulationExperiments/digits5/digits_5_ablation.py > /local/home/sfoell/GitHub/gdu-old/SimulationExperiments/digits5/2022-01-20/FT_0.log 2>&1 &
'''
...
init_gpu(gpu=2, memory=4000)

# file path to the location where the results are stored
res_file_dir = "/local/home/sfoell/GitHub/gdu-old/SimulationExperiments/digits5/DEBUG"
SOURCE_SAMPLE_SIZE = 25000
TARGET_SAMPLE_SIZE = 9000
img_shape = (32, 32, 3)

'''
nohup /local/home/sfoell/anaconda3/envs/gdu4dg/bin/python3.8 -u /local/home/sfoell/MTEC-IM-309/pub-gdu4dg/SimulationExperiments/experiment_4_digits/digits_5_ablation.py > /local/home/sfoell/MTEC-IM-309/pub-gdu4dg/SimulationExperiments/experiment_4_digits/visualizations.log 2>&1 &
'''
...
def digits_classification(method, TARGET_DOMAIN, single_best=False, single_source_domain=None,
    ...
    save_dir_path = os.path.join(save_dir_path, "E2E")
    create_dir_if_not_exists(save_dir_path)
    ...
    if domain_adaptation is False and fine_tune:
        ...
        for similarity_measure in ['projected', 'MMD', 'cosine_similarity']:
            ...
            x_data = {key: model.feature_extractor.predict(np.array(digits_data.x_train_dict[key][0:100])) for key in
                      digits_data.x_train_dict.keys()}
            ...
            embedded_domain_basis = TSNE(n_components=2, perplexity=15, n_iter=1000, random_state=1234).fit_transform(
                domain_basis_array)
            embedded_data = TSNE(n_components=2, perplexity=100, n_iter=5000, random_state=1234).fit_transform(
                x_data_array)
            embedding = np.concatenate((embedded_domain_basis, embedded_data))
            sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1],
                            hue=np.concatenate((domain_basis_index, x_data_index)).flatten(), palette="bright")
            plt.show()
            ...
            save_dir_path = os.path.join(save_dir_path, "SOURCE_COMBINED")
            create_dir_if_not_exists(save_dir_path)
                    ...
                    for batch_norm in [True]:  # , False]:
                        for bias in [False]:  # , True]:
                            ...
                            experiments.append({
                                ...
                                'batch_norm': True,
                                'bias': False,
                                'TARGET_DOMAIN': TEST_SOURCES, 'run' : i})
