...
class DigitsData(object):
    def __init__(self, test_size=SOURCE_SAMPLE_SIZE):
        self.x_train_dict, self.y_train_dict, self.x_test_dict, self.y_test_dict = load_digits(domains=["mnist", "mnistm", "svhn", "syn"], test_size=test_size)
...
def digits_classification(method, TARGET_DOMAIN, single_best=False, single_source_domain=None,
    ...
    if single_best and SOURCE_DOMAINS[0] == TARGET_DOMAIN[0].lower():
        ...
        source_domains = [source.lower() for source in SOURCE_DOMAINS if source.lower() != TARGET_DOMAIN[0].lower()]
        print(source_domains, TARGET_DOMAIN)
        x_source_tr = np.concatenate([data.x_train_dict[source] for source in source_domains], axis=0)
        y_source_tr = np.concatenate([data.y_train_dict[source] for source in source_domains], axis=0)
        ...
        x_source_te = np.concatenate([data.x_test_dict[source] for source in source_domains], axis=0)
        y_source_te = np.concatenate([data.y_test_dict[source] for source in source_domains], axis=0)
    ...
    domain_adaptation = method is not None

    assert not (domain_adaptation and fine_tune), "method should be None when fine_tune is True, got: {}".format(method)
...
#                                     max_sample_size=5000)
    ...
    if not domain_adaptation and fine_tune:
        ...
        for similarity_measure in ['cosine_similarity', 'MMD', 'projected']:
            ...
            if similarity_measure == 'projected':
                domain_adaptation_spec_dict['lambda_sparse'] = 0
            ...
            prediction_layer.add(DGLayer(domain_units=num_domains,
                                         ...
                                         similarity_measure=similarity_measure,
                                         orth_reg_method=orth_reg_method,
                                         lambda_sparse=lambda_sparse if similarity_measure == 'projected' else 0,
                                         lambda_OLS=lambda_OLS,
                                         lambda_orth=lambda_orth,
                                         ))
