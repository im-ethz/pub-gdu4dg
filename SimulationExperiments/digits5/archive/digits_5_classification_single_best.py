...
def digits_classification(method, TARGET_DOMAIN, single_best=True, single_source_domain=None,
    ...
    if architecture.lower() =="lenet":
        ...
        feature_extractor = conv_pool_conv_pool_fc_fc_softmax(dropout=dropout)
    ...
    if domain_adaptation:
        ...
        lambda_orth = domain_adaptation_spec_dict["lambda_orth"]
    ...
    if domain_adaptation is False and fine_tune and embedding:
        length = int(np.shape(x_source_tr)[0]*0.5)
        X_embedded_tr = model.feature_extractor.predict(x_source_tr[0:length])
        if i == 0:
        ...
        clusterer = hdbscan.HDBSCAN(min_samples=250, cluster_selection_epsilon=0.1, min_cluster_size=750)
        #clusterer = hdbscan.HDBSCAN(min_samples=1000, cluster_selection_epsilon=0.1, min_cluster_size=1000)
        ...
        clustered = (labels >= 0)
        print(np.sum(clustered) / clusterable_embedding.shape[0])
        plt.title('Estimated number of clusters: %d' % clusterer.labels_.max())
        plt.show()


        M = clusterer.labels_.max()
        print('Estimated number of clusters: %d' % M)
    ...
    if domain_adaptation is False and fine_tune:
        ...
        for similarity_measure in ['projected', 'MMD', 'cosine_similarity']:
            ...
            ###     PREDICTION LAYER
            #######################################
            #if similarity_measure == 'projected' and embedding:
            #    num_domains = domain_adaptation_spec_dict['num_domains'] = np.min([domain_adaptation_spec_dict['num_domains'], M])
            #if similarity_measure == 'projected' and embedding:
            #    num_domains = domain_adaptation_spec_dict['num_domains'] = M
            #else:
            num_domains = domain_adaptation_spec_dict["num_domains"]

            print('Number of parameter M: %d' % num_domains)

            domain_adaptation_spec_dict["similarity_measure"] = similarity_measure
            softness_param = domain_adaptation_spec_dict["softness_param"]
            domain_dim = domain_adaptation_spec_dict["domain_dim"]

            domain_adaptation_spec_dict['reg_method'] = reg_method = reg if similarity_measure == 'projected' else 'none'
...
def conv_pool_conv_pool_fc_fc_softmax(dropout=0.5):
    feature_exctractor = tf.keras.Sequential([
        ...
        , Dense(1024)
        , tf.keras.layers.ReLU()
    ], name='conv_pool_conv_pool_fc_fc_softmax')
...
if __name__ == "__main__":
    ...
    for i in [4]:
        experiments = []
        for method in [None]:
        #for method in ['projected','MMD', 'cosine_similarity']: #, 'MMD', 'cosine_similarity'
            for kernel in [None]:
                for TEST_SOURCES in [['mnistm'], ['mnist'], ['syn'], ['svhn'], ['usps']]:
                    for batch_norm in [True]:  # , False]:
                        for bias in [False]:  # , True]:
                            for single_source in [['mnistm'], ['mnist'], ['svhn'], ['syn'], ['usps']]:
                                select_mnistm = single_source[0] == 'mnist' and TEST_SOURCES[0] == 'mnistm'
                                select_mnist = single_source[0] == 'mnist' and TEST_SOURCES[0] == 'mnist'
                                select_svhn = single_source[0] == 'mnist' and TEST_SOURCES[0] == 'svhn'
                                select_usps = single_source[0] == 'mnist' and TEST_SOURCES[0] == 'usps'
                                select_syn = single_source[0] == 'mnist' and TEST_SOURCES[0] == 'syn'

                                if select_mnist or select_mnistm or select_svhn or select_usps or select_syn:
