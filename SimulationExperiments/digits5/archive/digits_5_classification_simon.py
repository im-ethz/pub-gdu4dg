...
init_gpu(gpu=1, memory=4000)

# file path to the location where the results are stored
res_file_dir = "/local/home/sfoell/NeurIPS/results/2021-05-22_source_only_ft"
...
if __name__ == "__main__":
    ...
    for i in [3]:
        experiments = []
        for method in [None]:
        #for method in ['projected']: #, 'MMD', 'cosine_similarity'
            for kernel in [None]:
                for TEST_SOURCES in [['mnistm'], ['mnist'], ['syn'], ['svhn'], ['usps']]:
                    for batch_norm in [True]:  # , False]:
                        for bias in [False]:  # , True]:
                            for single_source in [['mnistm'], ['mnist'], ['svhn'], ['syn'], ['usps']]:
                                select_mnistm = single_source[0] == 'syn' and TEST_SOURCES[0] == 'mnistm'
                                select_mnist = single_source[0] == 'mnistm' and TEST_SOURCES[0] == 'mnist'
                                select_svhn = single_source[0] == 'syn' and TEST_SOURCES[0] == 'svhn'
                                select_usps = single_source[0] == 'syn' and TEST_SOURCES[0] == 'usps'
                                select_syn = single_source[0] == 'svhn' and TEST_SOURCES[0] == 'syn'
